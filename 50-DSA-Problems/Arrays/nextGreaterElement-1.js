...
function nextGreaterElement_1(nums1, nums2) {
  // Below Time Complexity is O(nums2*2)
  let res = [];
  for (let i = 0; i < nums1.length; i++) {
    let val = nums1[i];
    let index = nums2.indexOf(val);
    console.log({ nums1: nums1[i], val, index, nextElem: nums2[index + 1] });
    let found = false;
  ...
  return res;
  // Todo: Complete in O(nums1.length + nums2.length) by Map & Stack Approach
}

console.log(nextGreaterElement_1([4, 1, 2], [1, 3, 4, 2])); // [-1,3,-1]
console.log(nextGreaterElement_1([2, 4], [1, 2, 3, 4])); // [3,-1]
console.log(nextGreaterElement_1([1, 3, 5, 2, 4], [6, 5, 4, 3, 2, 1, 7])); // [7,7,7,7,7]
